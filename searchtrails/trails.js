...
// function search() {
//     let trailQuery = document.querySelector('#searchinput').value;

//     let filterTrail = trails.filter(function(trail){
...
//   let sortedtrails = filterTrail.sort(compareTrails);
...
//   sortedtrails.forEach(function(trail){
//     renderTrail(trail);
//   })
// }
...
// Container for displaying trails
let trailContainer = document.querySelector('#description');
...
// Show a random trail on page load
function init() {
    let randomNum = Math.floor(Math.random() * trails.length);
    renderTrail(trails[randomNum]);
}
